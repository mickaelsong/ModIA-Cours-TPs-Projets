...
class DAN(nn.Module):
    ...
    def forward(self, ha, x, y):
        ...
        prior_mem = self.b(ha)
        prior_pdf = self.c(prior_mem)
        post_mem = self.a(torch.cat([prior_mem, y], dim=1))
        post_pdf = self.c(post_mem)

        logpdf_a = prior_pdf.log_prob(x).mean()
        logpdf_b = post_pdf.log_prob(x).mean()
        
        # TODO 2.4 rewrite loss for online mode
        ...
        with torch.no_grad():
            if logpdf_a is not None:
                self.scores["RMSE_b"].append(torch.mean(torch.norm(
                    post_pdf.mean - x, dim=1)*x.size(1)**-.5).item())
                self.scores["RMSE_a"].append(torch.mean(torch.norm(
                    prior_pdf.mean - x, dim=1)*x.size(1)**-.5).item())
...
class FcZero(nn.Module):
    ...
    def __init__(self, dim, deep, activation_classname):
        ...
        ### we set alpha to parameter of our model 
...
class Gaussian(nn.Module):
    ...
    def __init__(self, x_dim, vec, inds):
        ...
        ### TODO 1.5 Code optimized with functions torch.bmm and torch.triangular solve 
        cov = torch.bmm(self.Lambda, self.Lambda.transpose(1,2))
        cov_inv = torch.cholesky_inverse(self.Lambda)
    ...
    def log_prob(self, x):
        ...
        #### Code before TODO 1.5
        # for i in range(bs):
        #     mean_diff = x[i,:] - self.mu[i,:]
        #     cov_inv = self.cov_inv_[i,:,:]
        #     term_1 = -(1/2)*(mean_diff.T@cov_inv@mean_diff)
        ...
        bs = x.shape[0]
        logprob = torch.zeros(bs,1)
        mean_diff = x - self.mu.unsqueeze(1)
        L = self.Lambda
        z, _ = torch.triangular_solve(mean_diff.unsqueeze(-1), L, upper=False)

        term_1 = -0.5 * torch.matmul(z.transpose(-2, -1), z)
        term_2 = -self.cov_log_det_.squeeze(-1)
