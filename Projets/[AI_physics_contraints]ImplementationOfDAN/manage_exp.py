...
def pre_train_full(net,b_size,h_dim,x_dim,
    ...
    def closure0():
        ...
        print(f"Iteration {ite}: loss = {logpdf_a0.item()}")
...
def train_full(net, b_size, h_dim, x_dim,
    ...
    for t in range(T + 1):  # T + 1 to include the last time step
        # Propagate forward with the propogator module
        xt.append(x_prev)
        x_next = prop(x_prev).sample(sample_shape=torch.Size([1])).squeeze(0)

        # Observe the current state with the observer module
        yt.append(obs(x_next).sample(sample_shape=torch.Size([1])).squeeze(0))

        x_prev = x_next  # Update x_prev for the next time step
    ...
    xt = xt.cpu().detach().numpy().mean(axis=1)
    yt = yt.cpu().detach().numpy().mean(axis=1)
    mu = np.array(mu).mean(axis=1)
    plt.scatter(xt[:,0], xt[:,1], c="red", label="true")
    plt.scatter(yt[:,0], yt[:,1], c="green", marker="x", label="obs")
    plt.scatter(mu[:,0], mu[:,1], c="blue", label="analyse")
    plt.legend()
    plt.axis("equal")
...
def test(net, b_size, h_dim, x_dim,
    ...
    xt = torch.stack(xt,dim=0).cpu().detach().numpy().mean(axis=1)
    yt = torch.stack(yt,dim=0).cpu().detach().numpy().mean(axis=1)
    mu = np.array(mu).mean(axis=1)
    plt.scatter(xt[:, 0], xt[:, 1], c="red", label="true")
    plt.scatter(yt[:, 0], yt[:, 1], c="green", marker="x", label="obs")
    plt.scatter(mu[:, 0], mu[:, 1], c="blue", label="analyse")
